import { verifyRound, verifyCommitment } from './fairness.js';
...
function gameOptions(game) {
  switch (game) {
    ...
    case 'drill':
      return { rows: +$('drillRows').value, tilesPerRow: +$('drillTiles').value, trapsPerRow: +$('drillTraps').value };
    default:
      return {};
  }
}
...
function renderGrid(total, hits) {
  ...
  const cells = Array.from({ length: total }, (_, i) =>
    `<div class="cell ${hitSet.has(i) ? 'hit' : ''}">${i}</div>`).join('');
  return `<div class="mgrid" style="grid-template-columns: repeat(${cols}, 1fr)">${cells}</div>`;
}
function renderDrill(result) {
  return result.trapsByRow.map((traps, r) => {
    const trapSet = new Set(traps);
    const tiles = Array.from({ length: result.tilesPerRow }, (_, i) =>
      `<div class="drill-tile ${trapSet.has(i) ? 'trap' : ''}"></div>`).join('');
    return `<div class="drill-row"><span class="lbl">row ${r + 1}</span>${tiles}</div>`;
  }).join('');
}
function renderResult(game, nonce, result) {
  let body = '';
  if (game === 'mines') body = `<div class="value">Mines at: ${result.minePositions.join(', ')}</div>${renderGrid(result.totalCells, result.minePositions)}`;
  else if (game === 'moles') body = `<div class="value">Moles at: ${result.molePositions.join(', ')}</div>${renderGrid(result.totalCells, result.molePositions)}`;
  else if (game === 'dice') body = `<div class="value">Roll: ${result.roll.toFixed(2)}</div>`;
  else if (game === 'drill') body = `<div class="value">Trap layout</div>${renderDrill(result)}`;
...
async function run() {
  ...
  if (!serverSeed) { results.innerHTML = '<div class="error">Enter the revealed server seed.</div>'; return; }
  if (Number.isNaN(from) || Number.isNaN(to) || to < from) { results.innerHTML = '<div class="error">Check the nonce range.</div>'; return; }
  if (to - from > 500) { results.innerHTML = '<div class="error">Limit the range to 500 nonces at a time.</div>'; return; }
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = 'Verify round(s)';
  }
}
