...
/**
 * Generate Nth deterministic float for a given round using cached HMAC.
 */
function generateFloat(serverSeed: string, clientSeed: string, nonce: number, cursor: number): number {
  const message = `${clientSeed}:${nonce}:${cursor}`;
  const hmac = hmacCache.getOrHmac(serverSeed);
  hmac.reset();
  hmac.update(message);
  const hash = hmac.finalize();

  // Extract first 32 bits (8 hex chars) directly
  const firstWord = hash.words[0] >>> 0;
  return firstWord / 4294967296;
}
...
function validateCrashState(
  ...
  // Combine 32 bits from w0 and top 20 bits from w1 to get 52 bits
