...
// ─────────────────────────────────────────────────────────────────────────────
// Core crypto — optimized with cached HMAC & bitwise ops
// ─────────────────────────────────────────────────────────────────────────────

let _hmacInstance: ReturnType<typeof CryptoJS.algo.HMAC.create> | null = null;
let _currentServerSeed: string | null = null;

/**
 * Retrieve or initialize a cached HMAC instance for the given serverSeed.
 * Reduces object allocation overhead during high-frequency brute-forcing.
 */
function getHmac(serverSeed: string) {
  if (_hmacInstance && _currentServerSeed === serverSeed) {
    return _hmacInstance;
  }
  _hmacInstance = CryptoJS.algo.HMAC.create(CryptoJS.algo.SHA256, serverSeed);
  _currentServerSeed = serverSeed;
  return _hmacInstance;
}

/**
 * Generate Nth deterministic float for a given round.
 * Uses cached HMAC and bitwise operations to avoid expensive hex string conversions.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Crash — industry standard (optimized with cached HMAC & bitwise ops)
// ─────────────────────────────────────────────────────────────────────────────
