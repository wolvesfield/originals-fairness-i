...
app.post('/aim/mines', (req, res) => {
  ...
  // Hoist static cell array to avoid redundant allocations in the loop
  ...
  for (let n = nonce; n < nonce + cap; n++) {
    ...
    const minesSet = new Set(mines) // O(1) lookups instead of O(N) Array.includes()

    // Check if targetTiles contains any mines
    let allSafe = true;
    for (let i = 0; i < targetTiles.length; i++) {
      if (minesSet.has(targetTiles[i])) {
        allSafe = false;
        break;
      }
    }
...
app.post('/aim/keno', (req, res) => {
  ...
  for (let n = nonce; n < nonce + cap; n++) {
    ...
    const drawnSet = new Set(drawn) // O(1) lookups instead of O(N) Array.includes()
