...
// Cache the HMAC instance to avoid recreation overhead on every float generation
...
export function generateFloat(serverSeed: string, clientSeed: string, nonce: number, cursor: number, platform: 'stake' | 'roobet' = 'stake'): number {
  ...
  // Extract first 4 bytes using bitwise shift (equiv to taking first 8 hex chars and parsing)
