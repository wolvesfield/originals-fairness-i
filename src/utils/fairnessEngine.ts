...
// Single-value cache to avoid re-instantiating HMAC object in hot loops
let lastSeed: string | null = null
let lastHmac: any = null

function getHmac(seed: string) {
  if (seed !== lastSeed || !lastHmac) {
    lastSeed = seed
    lastHmac = CryptoJS.algo.HMAC.create(CryptoJS.algo.SHA256, seed)
  }
  return lastHmac
}
 ...
 * Optimized hot-path using bitwise operations instead of string allocations.
 */
export function generateFloat(serverSeed: string, clientSeed: string, nonce: number, cursor: number, platform: 'stake' | 'roobet' = 'stake'): number {
  ...
  // Directly access the first 32 bits (8 hex chars)
...
export function calculateCrashPoint(serverSeed: string, clientSeed: string, nonce: number): number {
  ...
  // Extract first 13 hex characters (52 bits) as an integer
  // words[0] = 32 bits, words[1] = 32 bits
  // We need all 32 bits from words[0] and top 20 bits from words[1]
