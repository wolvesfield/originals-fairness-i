...
export function calculateCrashPoint(serverSeed: string, clientSeed: string, nonce: number): number {
  ...
  // First 13 hex chars = 52 bits. Extract from first two 32-bit words:
  // First word gives 8 hex chars (32 bits). Need 5 more hex chars (20 bits) from second word.
  // We can do: (words[0] >>> 0) * 2^20 + (words[1] >>> 12)
