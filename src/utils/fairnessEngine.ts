...
function getHmacInstance(serverSeed: string) {
  if (serverSeed !== lastServerSeed || !cachedHmac) {
    ...
    // Pass the string key directly to avoid altering underlying bytes (see memory)
...
export function generateFloat(serverSeed: string, clientSeed: string, nonce: number, cursor: number, platform: 'stake' | 'roobet' = 'stake'): number {
  ...
  // Extract first 32 bits directly to avoid expensive hex string conversion and parseInt()
...
export function calculateCrashPoint(serverSeed: string, clientSeed: string, nonce: number): number {
  ...
  // We extract the first 32 bits from words[0] and the next 20 bits from words[1].
  const word0 = hash.words[0] >>> 0
  const word1 = hash.words[1] >>> 12 // take top 20 bits (32 - 12)
  const h = word0 * 1048576 + word1 // 1048576 = 2^20
