...
export function generateFloat(serverSeed: string, clientSeed: string, nonce: number, cursor: number, platform: 'stake' | 'roobet' = 'stake'): number {
  ...
  hmacCacheForFloat.update(message);
 ...
 */
const cellsArray = new Int32Array(256); // Safe size for mines up to 64 cells
...
export function generateMinePositions(
  ...
  if (activePlatform === 'roobet') {
    ...
    kenoDrawnArray.fill(0, 0, totalCells + 1); // We can reuse kenoDrawnArray as it's just a boolean map up to maxNum (or totalCells)
    const mines = new Array(mineCount);
    let resultIdx = 0;
    let cursor = 0;
    while (resultIdx < mineCount) {
      const float = generateFloat(serverSeed, clientSeed, nonce, cursor, activePlatform)
      const mine = Math.floor(float * totalCells)
      if (kenoDrawnArray[mine] === 0) {
        kenoDrawnArray[mine] = 1;
        mines[resultIdx++] = mine;
      }
      cursor++
    }
 ...
 */
const kenoDrawnArray = new Uint8Array(256); // Safe size since maxNum is typically 40
...
export function generateKenoNumbers(
  ...
  kenoDrawnArray.fill(0, 0, maxNum + 2); // clear only what we might use
